...
def predict(knowladge_based_chat_llm,input,
            top_k,
            history_len,
            temperature,
            top_p,
            history=None):
    print(input,top_k,history_len,temperature,top_p,history)
    if history == None:
        history = []
...
knowladge_based_chat_llm = init_model()
...
data = st.sidebar.file_uploader("上传文本文件", type=["txt", "md", "docx"])
...
if st.button("发送", key="predict"):
    with st.spinner("AI正在思考，请稍等........"):
        st.session_state["state"] = predict(knowladge_based_chat_llm,prompt_text, top_k,history_len, temperature, top_p, st.session_state["state"])
